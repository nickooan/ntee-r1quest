...
import { summarizeAiConversationActivity } from "../src/views/terminal/ai-session.ts"
import { buildFilePaneLayout } from "../src/views/terminal/file-content.tsx"

describe("terminal app view", () => {
  test("summarizes active and idle AI conversations", () => {
    expect(
  ...
  test("treats old tool conversations as background even with recent updates", () => {
    expect(
      summarizeAiConversationActivity(
        [
          {
  ...
  test("counts terminal-backed background sessions from completed conversations", () => {
    expect(
      summarizeAiConversationActivity(
        [
          {
