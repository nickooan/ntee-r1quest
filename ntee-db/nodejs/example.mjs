...
db.put('call:1', JSON.stringify({ kind: 'request', url: '/orders' }), { traceId: 'T1' });
db.put('call:2', JSON.stringify({ kind: 'request', url: '/items' }), { traceId: 'T1' });
db.put('call:3', JSON.stringify({ kind: 'history', url: '/x' }), { traceId: 'T2' });
...
console.log(
  ...
  db.searchByIndex('kind', 'request').map((r) => ({ key: r.key, value: JSON.parse(r.value.toString()) })),
);

db.close();

// Reopen — state is recovered from disk.
db = NteeDB.open(dir, { indexes: [{ name: 'traceId', kind: 'string' }, { name: 'kind', kind: 'string' }] });
console.log('after reopen, get call:1:', JSON.parse(db.get('call:1').toString()));
db.close();
